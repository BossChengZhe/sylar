...
// sylar::ConfigVar<float>::ptr g_int_valx_config =
//     sylar::Config::Lookup("system.port", (float)8080, "system port");

sylar::ConfigVar<float>::ptr g_float_val_config =
    sylar::Config::Lookup("system.val", (float)10.2f, "system val");

sylar::ConfigVar<std::vector<int>>::ptr g_int_vec_val_config =
    sylar::Config::Lookup("system.int_vec", std::vector<int>{1, 2}, "system int vec");

sylar::ConfigVar<std::list<int>>::ptr g_int_list_val_config =
    sylar::Config::Lookup("system.int_list", std::list<int>{1, 2}, "system int list");

sylar::ConfigVar<std::set<int>>::ptr g_int_set_val_config =
    sylar::Config::Lookup("system.int_set", std::set<int>{1, 2}, "system int set");

sylar::ConfigVar<std::unordered_set<int>>::ptr g_int_uset_val_config =
    sylar::Config::Lookup("system.int_uset", std::unordered_set<int>{1, 2}, "system int uset");

sylar::ConfigVar<std::map<std::string, int>>::ptr g_str_int_map_val_config =
    sylar::Config::Lookup("system.str_int_map", std::map<std::string, int>{{"k", 2}}, "system str int map");

sylar::ConfigVar<std::unordered_map<std::string, int>>::ptr g_str_int_umap_val_config =
...
void print_yaml(const YAML::Node &node, int level)
{
    if(node.IsScalar()) {
        ...
        for (auto iter = node.begin(); iter != node.end(); iter++){
            SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "Is Map " << std::string(level*4, ' ') <<
                iter->first << " - " << iter->second.Type() << " - " << level;
            print_yaml(iter->second, level + 1);
        }
    }
    else if(node.IsSequence()) {
        for (size_t i = 0; i < node.size(); i++){
            SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "Is Seq " << std::string(level*4, ' ') 
                << i << " - " << node[i].Type() << " - " << level;
...
#define XX_PM(g_var, prefix)               \
    ...
    XX_PM(g_person_map, "class.map befor");
    YAML::Node root = YAML::LoadFile("/home/cheng/Code/sylar/bin/conf/test.yml");
    sylar::Config::LoadFromYaml(root);
